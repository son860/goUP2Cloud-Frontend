      ...
      function($q, $injector, $location) {
        return {
          ...
          responseError: function responseErrorCallback(response) {
            ...
            if (response.status === 401){
                if (response.data){
                  if (response.data.errors){
                    angular.forEach(response.data.errors, function(value) {                
                      message += '<p class="text-small">' + value + '</p>';
                    });
            ...
            } else if (response.data && response.data.invalidAttributes) {
              angular.forEach(response.data.invalidAttributes, function(value, key) {                
                message = '<strong><p>' + key + '</p></strong>';
                angular.forEach(value, function(value2) {
                  message += '<p class="text-small">' + value2.message + '</p>';
                });       
              ...
              angular.forEach(response.data.errors.full_messages, function(value, key) {                
                  toastr['error'](value);           
              });
            } else {
